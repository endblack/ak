  ...
  console.log(anu)
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isPrem = checkprem(sender) || isOwner
...
case 'cassino':
		...
		break
		case 'tem':
		gfs = ms(gettime(sender) - Date.now())
		reply(`.... ${gfs.days} ${gfs.hours} ${gfs.minutes}`)
		break
		case 'infome':
		case 'informe':
case 'perfil':
...
break
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        request({
                            url: `https://api.lolhuman.xyz/api/convert/towebpwround?apikey=ea1c735e809aedcb3b42793a}`,
                            method: 'POST',
                            formData: {
...
\`\`\`Velocidade: ${latensi.toFixed(4)} Segundos\`\`\``
				...
				if (!isRegistered) return reply(ind.noregis(pushname))  
					...
					break
					case 'apk':
		arqu = fs.readFileSync('./apk/RAR_5.91.build95.apk')
		client.sendMessage(from, arqu, document, {mimetype: 'apk', filename: 'RAR_5.91.build95.apk', quoted: mek})
					break
					case 'tt':
				if (!isRegistered) return reply(ind.noregis(pushname))  
					rate = body.slice(1)
